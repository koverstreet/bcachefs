...
static int total_duration_cmp(const void *_l, const void *_r)
{
...
static int max_duration_cmp(const void *_l, const void *_r)
{
	const struct sched_wakeup_stats * const *l = _l;
	const struct sched_wakeup_stats * const *r = _r;

	return -cmp_int((*l)->stats.max_duration, (*r)->stats.max_duration);
}
				...
				cmp_func_t cmp_func)
{
	...
	sort(iter->d, nr, sizeof(iter->d[0]), cmp_func, NULL);
	return iter;
}
...
static void *sched_wakeup_total_duration_start(struct seq_file *m, loff_t *pos)
{
	return sched_wakeup_start(m, pos, total_duration_cmp);
}

static int sched_wakeups_total_duration_open(struct inode *inode, struct file *filp)
{
	static const struct seq_operations sched_wakeup_ops = {
		.start	= sched_wakeup_total_duration_start,
...
static const struct file_operations sched_wakeups_total_duration_fops = {
	.open		= sched_wakeups_total_duration_open,
	.read		= seq_read,
	.llseek		= seq_lseek,
	.release	= seq_release,
};

static void *sched_wakeup_max_duration_start(struct seq_file *m, loff_t *pos)
{
	return sched_wakeup_start(m, pos, max_duration_cmp);
}

static int sched_wakeups_max_duration_open(struct inode *inode, struct file *filp)
{
	static const struct seq_operations sched_wakeup_ops = {
		.start	= sched_wakeup_max_duration_start,
		.stop	= sched_wakeup_stop,
		.next	= sched_wakeup_next,
		.show	= sched_wakeup_show,
	};
...
static const struct file_operations sched_wakeups_max_duration_fops = {
	.open		= sched_wakeups_max_duration_open,
	.read		= seq_read,
	.llseek		= seq_lseek,
	.release	= seq_release,
};
...
static int sched_wakeup_backtrace_init(void)
{
	...
	debugfs_create_file("wakeups_total_duration",	0444, debugfs_sched, NULL, &sched_wakeups_total_duration_fops);
	debugfs_create_file("wakeups_max_duration",	0444, debugfs_sched, NULL, &sched_wakeups_max_duration_fops);
	return 0;
}
