						...
						bool try)
{
	...
	if (type != SIX_LOCK_intent && lock->readers) {
		 ...
		 * Percpu reader mode:
		 *
		 * The basic idea behind this algorithm is that you can
		 * implement a lock between two threads without any atomics,
		 * just memory barriers:
		 *
		 * For two threads you'll need two variables, one variable for
		 * "thread a has the lock" and another for "thread b has the
		 * lock".
		 *
		 * To take the lock, a thread sets its variable indicating that
		 * it holds the lock, then issues a full memory barrier, then
		 * reads from the other thread's variable to check if the other
		 * thread thinks it has the lock. If we raced, we backoff and
		 * retry/sleep.
		 */
