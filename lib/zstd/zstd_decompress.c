...
struct ZSTD_DCtx_s {
    ...
    size_t litSize;
    size_t rleSize;
    u8 litBuffer[ZSTD_BLOCKSIZE_ABSOLUTEMAX + WILDCOPY_OVERLENGTH];
    u8 headerBuffer[ZSTD_FRAMEHEADERSIZE_MAX];
...
size_t ZSTD_decompressBegin(ZSTD_DCtx* dctx)
{
    ...
    dctx->dictID = 0;
    BUILD_BUG_ON(sizeof(dctx->rep) != sizeof(repStartValue));
...
ZSTD_DCtx* ZSTD_createDCtx(void)
{
    ...
    dctx = kmalloc(sizeof(ZSTD_DCtx), GFP_KERNEL);
    if (!dctx)
	    return NULL;
...
static size_t ZSTD_setRleBlock(void* dst, size_t dstCapacity, const void* src, size_t srcSize, size_t regenSize)
{
    ...
    memset(dst, *(const u8*)src, regenSize);
    return regenSize;
}
                                 ...
                                 const FSE_decode_t4* defaultTable, u32 flagRepeatTable)
{
    ...
    case set_rle :
        ...
        if ( (*(const u8*)src) > max) return ERROR(corruption_detected);
        FSE_buildDTable_rle(DTableSpace, *(const u8*)src);
...
size_t ZSTD_decodeSeqHeaders(ZSTD_DCtx* dctx, int* nbSeqPtr,
                             const void* src, size_t srcSize)
{
    const u8* const istart = (const u8* const)src;
...
noinline
size_t ZSTD_execSequenceLast7(u8* op,
                              u8* const oend, seq_t sequence,
                              const u8** litPtr, const u8* const litLimit,
                              const u8* const base, const u8* const vBase, const u8* const dictEnd)
{
    u8* const oLitEnd = op + sequence.litLength;
    size_t const sequenceLength = sequence.litLength + sequence.matchLength;
    u8* const oMatchEnd = op + sequenceLength;   /* risk : address space overflow (32-bits) */
    u8* const oend_w = oend - WILDCOPY_OVERLENGTH;
    const u8* const iLitEnd = *litPtr + sequence.litLength;
    const u8* match = oLitEnd - sequence.offset;
...
__always_inline
size_t ZSTD_execSequence(u8* op,
                                u8* const oend, seq_t sequence,
                                const u8** litPtr, const u8* const litLimit,
                                const u8* const base, const u8* const vBase, const u8* const dictEnd)
{
...
{
    const u8* ip = (const u8*)seqStart;
    const u8* const iend = ip + seqSize;
    u8* const ostart = (u8* const)dst;
    u8* const oend = ostart + maxDstSize;
    u8* op = ostart;
    const u8* litPtr = dctx->litPtr;
    const u8* const litEnd = litPtr + dctx->litSize;
    const u8* const base = (const u8*) (dctx->base);
    const u8* const vBase = (const u8*) (dctx->vBase);
    const u8* const dictEnd = (const u8*) (dctx->dictEnd);
    int nbSeq;
...
__always_inline
size_t ZSTD_execSequenceLong(u8* op,
                                u8* const oend, seq_t sequence,
                                const u8** litPtr, const u8* const litLimit,
                                const u8* const base, const u8* const vBase, const u8* const dictEnd)
{
...
#define ADVANCED_SEQS 4
        ...
        seqState.gotoDict = (long)(dictEnd - base);
        CHECK_E(BIT_initDStream(&seqState.DStream, ip, iend-ip), corruption_detected);
        FSE_initDState(&seqState.stateLL, &seqState.DStream, dctx->LLTptr);
        FSE_initDState(&seqState.stateOffb, &seqState.DStream, dctx->OFTptr);
        FSE_initDState(&seqState.stateML, &seqState.DStream, dctx->MLTptr);
...
size_t ZSTD_generateNxBytes(void* dst, size_t dstCapacity, u8 byte, size_t length)
{
...
ZSTD_DDict* ZSTD_createDDict_advanced(const void* dict, size_t dictSize, unsigned byReference)
{
    {   ZSTD_DDict* const ddict = kmalloc(sizeof(ZSTD_DDict), GFP_KERNEL);
        ...
        if ((byReference) || (!dict) || (!dictSize)) {
            ...
            if (!internalBuffer) { kfree(dctx); kfree(ddict); return NULL; }
...
{
    return ZSTD_createDDict_advanced(dict, dictSize, 0);
}
...
{
    return ZSTD_createDDict_advanced(dictBuffer, dictSize, 1);
}
...
struct ZSTD_DStream_s {
    ...
    size_t outEnd;
    size_t blockSize;
    u8 headerBuffer[ZSTD_FRAMEHEADERSIZE_MAX];   /* tmp buffer to store frame header */
    size_t lhSize;
    ZSTD_customMem customMem;
    void* legacyContext;
...
ZSTD_DStream* ZSTD_createDStream(void)
{
    ...
    zds = kzalloc(sizeof(ZSTD_DStream), GFP_KERNEL);
    if (zds==NULL) return NULL;
    zds->dctx = ZSTD_createDCtx();
		...
		size_t const hSize = ZSTD_getFrameParams(&zds->fParams, zds->headerBuffer, zds->lhSize);
            ...
            {   size_t const blockSize = MIN(zds->fParams.windowSize, ZSTD_BLOCKSIZE_ABSOLUTEMAX);
                ...
                if (zds->inBuffSize < blockSize) {
                    ...
                    zds->inBuff = kmalloc(blockSize, GFP_KERNEL);
                    if (zds->inBuff == NULL) return ERROR(memory_allocation);
                }
                if (zds->outBuffSize < neededOutSize) {
                    kfree(zds->outBuff);
                    zds->outBuffSize = neededOutSize;
                    zds->outBuff = kmalloc(neededOutSize, GFP_KERNEL);
