...
static void __bkey_cached_free(struct pending_rcu_items *pending, struct rcu_head *rcu)
{
			     ...
			     struct bkey_cached *ck)
{
	...
	bch2_pending_rcu_item_enqueue(&bc->pending[pcpu_readers], &ck->rcu);
...
bkey_cached_alloc(struct btree_trans *trans, struct btree_path *path, unsigned key_u64s)
{
	...
	struct bkey_cached *ck = container_of_or_null(
				bch2_pending_rcu_item_dequeue(&bc->pending[pcpu_readers]),
	...
	ck = container_of_or_null(bch2_pending_rcu_item_dequeue_from_all(&bc->pending[pcpu_readers]),
				  struct bkey_cached, rcu);
	if (ck)
		goto lock;
lock:
...
void bch2_fs_btree_key_cache_exit(struct btree_key_cache *bc)
{
	...
	bch2_pending_rcu_items_exit(&bc->pending[0]);
	bch2_pending_rcu_items_exit(&bc->pending[1]);
}
...
int bch2_fs_btree_key_cache_init(struct btree_key_cache *bc)
{
	...
	if (bch2_pending_rcu_items_init(&bc->pending[0],
					&c->btree_trans_barrier, __bkey_cached_free) ||
	    bch2_pending_rcu_items_init(&bc->pending[1],
					&c->btree_trans_barrier, __bkey_cached_free))
