					    ...
					    struct btree_update *as)
{
	...
	darray_for_each(as->new_nodes, i) {
		...
		try(bch2_key_trigger_new(trans, as->btree_id, i->level + 1,
					 bkey_i_to_s_c(&i->key),
					 BKEY_BTREE_PTR_U64s_MAX,
					 BTREE_TRIGGER_transactional));
...
out:
	...
	time_stats_update(&c->times[dsts.nr == 2
				   ...
				   enum bch_write_flags write_flags)
{
	...
	if (unlikely(!percpu_down_read_trylock(&c->gc.done_lock))) {
		ret = drop_locks_do(trans, (percpu_down_read(&c->gc.done_lock), 0));
		if (ret)
			goto out;
	}
