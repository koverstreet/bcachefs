				     ...
				     struct found_btree_node *r)
{
	...
	if (!cmp || (cmp1 && cmp2 && cmp1 != cmp2)) {
		...
		bch2_found_btree_node_to_text(&buf, c, l);
		bch2_found_btree_node_to_text(&buf, c, r);
				...
				struct btree *b, struct bio *bio, u64 offset)
{
	...
	bch2_found_btree_node_to_text(&buf, c, &n);
	prt_newline(&buf);

	int ret = bch2_btree_node_read_done(c, ca, b, NULL, &buf);

	bch_verbose_dev(ca, "attempted to read, ret %s\n%s", bch2_err_str(ret), buf.buf);
