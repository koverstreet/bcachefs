...
static enum merge_result bch_extent_merge(struct cache_set *, struct btree_keys *,
...
static bool bch_ptr_normalize(struct cache_set *c, struct btree_keys *bk,
			      struct bkey_s k)
{
				    ...
				    struct bucket_stats_cache_set *stats)
{
	...
	if (bkey_cmp(committed_pos, iter->pos) > 0) {
		...
		if (!(trans->flags & BTREE_INSERT_JOURNAL_REPLAY) &&
		    ...
		    bkey_extent_is_compressed(c, bkey_i_to_s_c(insert->k))) {
			  ...
			  struct bucket_stats_cache_set *stats)
{
	...
	if (k.k && bkey_cmp(*committed_pos, bkey_start_pos(k.k)) < 0) {
		...
		switch (__extent_insert_advance_pos(trans, insert,
						    committed_pos,
						    bkey_start_pos(k.k),
						    bkey_s_c_null,
						    res, stats)) {
