				       ...
				       u64 start, u64 end)
{
	...
	stats->duration_stats_ewm = mean_and_variance_ewm_update(stats->duration_stats_ewm, duration);
...
void bch2_time_stats_to_text(struct printbuf *out, struct time_stats *stats)
{
	...
	prt_tab(out);
	pr_time_units(out, get_mean(stats->duration_stats));

	prt_newline(out);
	prt_printf(out, "stddev duration:");
	prt_tab(out);
	pr_time_units(out, get_stddev(stats->duration_stats));
	...
	prt_printf(out, "avg duration:");
	prt_tab(out);
	pr_time_units(out, get_ewm_mean(stats->duration_stats_ewm));

	prt_newline(out);
	prt_printf(out, "stddev duration:");
	prt_tab(out);
	pr_time_units(out, get_ewm_stddev(stats->duration_stats_ewm));
