			       ...
			       s64 *disk_sectors_delta)
{
	...
	return __bch2_sum_sector_overwrites(trans, extent_iter, new->k.p,
				...
				bch2_bkey_nr_ptrs_allocated(c, bkey_i_to_s_c(new)));
}
...
static int bch2_write_index_default(struct bch_write_op *op)
{
	...
	for_each_keylist_key(keys, k) {
		compressed |= bch2_bkey_sectors_compressed(c, bkey_i_to_s_c(k));
		new_replicas = min(new_replicas, bch2_bkey_replicas(c, bkey_i_to_s_c(k)));
		new_ptrs_allocated = min(new_ptrs_allocated, bch2_bkey_nr_ptrs_allocated(c, bkey_i_to_s_c(k)));
	}
	...
	while (true) {
		...
		for_each_keylist_key(keys, k) {
			atomic_end = k->k.p;
			ret = bch2_extent_atomic_end(&iter, &nr_iters, &atomic_end);
			if (ret < 0)
				goto err1;
			if (ret) {
				ret = 0;
		...
		bch2_trans_copy_iter(&insert_iter, &iter);

		for_each_keylist_key(keys, k) {
			unsigned n_buf_u64s = k->k.u64s + 1 + BCH_REPLICAS_MAX;
			struct bkey_i *n = bch2_trans_kmalloc_nomemzero(trans,
			...
			if (k != bch2_keylist_front(keys))
				bch2_btree_iter_set_pos(&insert_iter, bkey_start_pos(&n->k));

			ret = bch2_trans_update_buf(trans, &insert_iter, n, n_buf_u64s, 0);
			if (ret)
				goto err2;
		...
		ret = bch2_trans_commit(trans, &op->res, NULL,
					BCH_TRANS_COMMIT_no_check_rw|
					BCH_TRANS_COMMIT_no_enospc);
err2:
		bch2_trans_iter_exit(&insert_iter);
err1:
		...
		bch2_cut_front(c, atomic_end, bch2_keylist_front(keys));
	}
