...
static int unset_rebalance_bp(struct btree_trans *trans, enum btree_id work_btree,
			      enum btree_id btree, unsigned level, struct bkey_s_c k)
{
	...
	CLASS(btree_iter, iter)(trans, BTREE_ID_rebalance_scan, POS(1, idx), BTREE_ITER_intent);
			    ...
			    enum btree_id btree, unsigned level, struct bkey_s k)
{
	...
	try(bch2_bkey_get_empty_slot(trans, &iter, BTREE_ID_rebalance_scan, POS(1, U64_MAX)));
	...
	if (old_btree != new_btree) {
		if (old_btree)
			try(!level
			    ...
			    : unset_rebalance_bp(trans, old_btree, btree, level, old));
...
static int check_rebalance_work_btree_key(struct btree_trans *trans, struct btree_iter *iter, struct bkey_s_c k)
{
	...
	if (fsck_err_on(rb_btree && !rb_idx,
		...
		try(set_rebalance_bp(trans, rb_btree, iter->btree_id, iter->min_depth, bkey_i_to_s(n)));
		return 0;
	}
	...
	if (rb_idx) {
		...
		if (bp_k.k->type != KEY_TYPE_backpointer || memcmp(bp_k.v, &bp, sizeof(bp))) {
			...
			if (bp_k.k->type != KEY_TYPE_backpointer) {
				...
				bch2_trans_iter_exit(&rb_iter);
				try(bch2_bkey_get_empty_slot(trans, &rb_iter, BTREE_ID_rebalance_scan, POS(1, U64_MAX)));
...
int bch2_check_rebalance_work(struct bch_fs *c)
{
	...
	CLASS(btree_iter, rb_w)(trans, BTREE_ID_rebalance_work, POS_MIN,
				BTREE_ITER_prefetch|BTREE_ITER_all_snapshots);
	CLASS(btree_iter, rb_h)(trans, BTREE_ID_rebalance_work_hipri, POS_MIN,
				BTREE_ITER_prefetch|BTREE_ITER_all_snapshots);
	CLASS(btree_iter, rb_p)(trans, BTREE_ID_rebalance_work_pending, POS_MIN,
				BTREE_ITER_prefetch|BTREE_ITER_all_snapshots);
	...
	int ret;
	while (!(ret = commit_do(trans, NULL, NULL, BCH_TRANS_COMMIT_no_enospc,
				 check_rebalance_work_one(trans, &extent_iter, &rb_w, &rb_h, &rb_p,
							  &snapshot_io_opts, &last_flushed, &cur_pos))) &&
	       !bpos_eq(cur_pos, SPOS_MAX)) {
		progress_update_iter(trans, &progress, &rb_w);
