...
static void bch2_dev_io_ref_start(struct bch_dev *ca, int rw)
{
	lockdep_assert_held(&ca->io_ref_statechange_lock);

	enumerated_ref_start(&ca->io_ref[rw]);
}
				  ...
				  struct bch_sb_handle *sb, struct printbuf *err)
{
	...
	if (!is_power_of_2(bs)) {
		prt_printf(err, "cannot online: logical block size %u not a power of 2\n", bs);
		return -BCH_ERR_device_bad_blocksize;
	}

	if (bs > BCH_BLOCK_SIZE_MAX) {
		prt_printf(err, "cannot online: logical block size %u too big\n", bs);
		return -BCH_ERR_device_bad_blocksize;
	}
	...
	if (!is_power_of_2(bs)) {
		prt_printf(err, "cannot online: physical block size %u not a power of 2", bs);
		return -BCH_ERR_device_bad_blocksize;
	}

	if (bs > BCH_BLOCK_SIZE_MAX) {
		prt_printf(err, "cannot online: physical block size %u too big", bs);
		return -BCH_ERR_device_bad_blocksize;
	}

	if (!IS_ENABLED(CONFIG_TRANSPARENT_HUGEPAGE) &&
	   bs > PAGE_SIZE) {
		prt_printf(err, "cannot mount bs > ps filesystem without CONFIG_TRANSPARENT_HUGEPAGE");
		return -BCH_ERR_device_bad_blocksize;
	}
	...
	if (ca->block_bits_log > ca->block_bits_phys) {
		prt_printf(err, "cannot online: logical block size %u > physical block size %u",
...
static int bch2_fs_bdev_thaw(struct block_device *bdev)
{
	...
	if (ca->disk_sb.bdev &&
		...
		bch2_dev_io_ref_start(ca, READ);
		if (ca->mi.state == BCH_MEMBER_STATE_rw)
			bch2_dev_io_ref_start(ca, WRITE);
	}
	--ca->frozen;
