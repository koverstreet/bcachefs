	...
	*sectors = insert ? bp.v.bucket_len : -(s64) bp.v.bucket_len;
	...
	if (unlikely(!ca)) {
		if (insert && p.ptr.dev != BCH_SB_MEMBER_INVALID)
			return bch_err_throw(c, trigger_pointer_to_missing_dev);
		return 0;
	}
	...
	if (flags & BTREE_TRIGGER_transactional) {
		...
		try(__mark_pointer(trans, ca, k, &p, *sectors, bp.v.data_type, &a->v, flags));
	...
	if (flags & BTREE_TRIGGER_gc) {
		...
		scoped_guard(bucket_lock, g) {
			...
			try(__mark_pointer(trans, ca, k, &p, *sectors, bp.v.data_type, &new, flags));
			...
			enum btree_iter_update_trigger_flags flags)
{
	...
	scoped_guard(bucket_lock, g) {
		...
		if (g->data_type && g->data_type != data_type) {
			int ret = probe_data_type(ca, b);
			if (ret < 0)
				return ret;
