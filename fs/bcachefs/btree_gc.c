...
static void ptr_gens_recalc_oldest(struct bch_fs *c, struct bkey_s_c k,
				   u8 *max_stale)
{
	const struct bch_extent_ptr *ptr;
...
static int ptr_gens_check(struct bch_fs *c, struct bkey_s_c k)
{
	const struct bch_extent_ptr *ptr;
	int ret = 0;
			    ...
			    u8 *max_stale, bool initial)
{
	...
	ptr_gens_recalc_oldest(c, k, max_stale);
fsck_err:
	return ret;
}
