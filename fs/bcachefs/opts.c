...
u64 BCH2_NO_MEMBER_OPT(const struct bch_member *sb)
{
	BUG();
}

void SET_BCH2_NO_MEMBER_OPT(struct bch_member *sb, u64 v)
{
	BUG();
}
...
#define mask_if_eq(_p, _v)	(_p != _v ? _p : NULL)

#define filter_if_type_and_neq(_p, _v)					\
	mask_if_eq(type_compatible_or_null(_p, _v), _v)
...
#define x(_name, _bits, _flags, _type, _sb_opt, _default, _hint, _help)	\
	[Opt_##_name] = {						\
		...
		.hint		= _hint,				\
		.help		= _help,				\
		.get_sb		= filter_if_type_and_neq(_sb_opt,	BCH2_NO_SB_OPT),	\
		.set_sb		= filter_if_type_and_neq(SET_##_sb_opt,	SET_BCH2_NO_SB_OPT),	\
		.get_member	= filter_if_type_and_neq(_sb_opt,	BCH2_NO_MEMBER_OPT),	\
		.set_member	= filter_if_type_and_neq(SET_##_sb_opt,	SET_BCH2_NO_MEMBER_OPT),	\
		_type							\
	},
