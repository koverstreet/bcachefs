...
static void bch2_inode_init_owner(struct user_namespace *mnt_userns,
				  struct bch_inode_unpacked *inode_u,
				  const struct inode *dir, umode_t mode)
{
	...
	inode_u->bi_uid		= from_kuid(mnt_userns, uid);
	inode_u->bi_gid		= from_kgid(mnt_userns, gid);
	      ...
	      umode_t mode, dev_t rdev, bool tmpfile)
{
	...
	bch2_inode_init_owner(mnt_userns, &inode_u, &dir->v, mode);
				       ...
				       void *p)
{
	...
	if (ia_valid & ATTR_UID)
		bi->bi_uid = from_kuid(s->ns, s->attr->ia_uid);
	if (ia_valid & ATTR_GID)
		bi->bi_gid = from_kgid(s->ns, s->attr->ia_gid);
	...
	if (ia_valid & ATTR_MODE) {
		...
		if (!in_group_p(gid) &&
		    !capable_wrt_inode_uidgid(s->ns, &inode->v, CAP_FSETID))
			mode &= ~S_ISGID;
		bi->bi_mode = mode;
	}
				...
				struct iattr *iattr)
{
	...
	struct inode_write_setattr s = { iattr, mnt_userns };
	int ret;
