						    ...
						    umode_t mode, dev_t rdev)
{
	...
	if (default_acl) {
		ret = bch2_set_acl(mnt_userns, &inode->v, default_acl, ACL_TYPE_DEFAULT);
		if (unlikely(ret))
			goto err;
	}

	if (acl) {
		ret = bch2_set_acl(mnt_userns, &inode->v, acl, ACL_TYPE_ACCESS);
		if (unlikely(ret))
			goto err;
	}
...
static int __bch2_create(struct user_namespace *mnt_userns,
			 ...
			 umode_t mode, dev_t rdev)
{
	...
	inode = bch2_vfs_inode_create(c, mnt_userns, dir, mode, rdev);
...
{
	return __bch2_create(mnt_userns, to_bch_ei(vdir), dentry, mode|S_IFREG, 0);
}
		      ...
		      struct inode *vdir, struct dentry *dentry, umode_t mode)
{
	...
	ret = __bch2_create(mnt_userns, dir, dentry, mode|S_IFDIR, 0);
	if (unlikely(ret))
		return ret;
...
static int bch2_mknod(struct user_namespace *mnt_userns,
		      ...
		      umode_t mode, dev_t rdev)
{
	return __bch2_create(mnt_userns, to_bch_ei(vdir), dentry, mode, rdev);
}
...
static int bch2_rename2(struct user_namespace *mnt_userns,
			struct inode *old_vdir, struct dentry *old_dentry,
				...
				struct iattr *iattr)
{
	...
	if (c->opts.usrquota &&
		...
		qid.q[QTYP_USR] = from_kuid(mnt_userns, iattr->ia_uid),
		qtypes |= 1 << QTYP_USR;
	}
	...
	if (c->opts.grpquota &&
		...
		qid.q[QTYP_GRP] = from_kgid(mnt_userns, iattr->ia_gid);
		qtypes |= 1 << QTYP_GRP;
	}
	...
	setattr_copy(mnt_userns, &inode->v, iattr);
...
out_unlock:
	...
	if (!ret &&
		...
		ret = posix_acl_chmod(mnt_userns, &inode->v, inode->v.i_mode);
			...
			struct inode *vdir, struct file *file, umode_t mode)
{
	...
	inode = bch2_vfs_inode_create(c, mnt_userns, dir, mode, 0);
	if (unlikely(IS_ERR(inode)))
		return PTR_ERR(inode);

	d_tmpfile(file, &inode->v);
